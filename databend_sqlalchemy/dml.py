...
class _CopyIntoBase(UpdateBase):
    ...
    def __init__(self, target: ['TableClause', 'StageClause', '_StorageClause'], from_, file_format: 'CopyFormat' = None,
...
class CopyIntoLocation(_CopyIntoBase):
    inherit_cache = False
    def __init__(self, *, target: ['StageClause', '_StorageClause'], from_, file_format: 'CopyFormat' = None, options: 'CopyIntoLocationOptions' = None):
        super().__init__(target, from_, file_format, options)


class CopyIntoTable(_CopyIntoBase):
    inherit_cache = False
    def __init__(self, *, target: [TableClause], from_: ['StageClause', '_StorageClause', 'FileColumnClause'],
                 files: list = None, pattern: str = None, file_format: 'CopyFormat' = None, options: 'CopyIntoTableOptions' = None):
        super().__init__(target, from_, file_format, options)
        self.files = files
...
class CopyIntoLocationOptions(_CopyIntoOptions):
    ...
    def __init__(self, *, single: bool = None, max_file_size_bytes: int = None, overwrite: bool = None,
                include_query_id: bool = None, use_raw_path: bool = None):
        super().__init__()
...
class CopyIntoTableOptions(_CopyIntoOptions):
    ...
    def __init__(self, *, size_limit: int = None, purge: bool = None, force: bool = None,
                disable_variant_check: bool = None, on_error: str = None, max_files: int = None,
                return_failed_only: bool = None, column_match_mode: str = None):
        super().__init__()
        ...
        if not isinstance(disable_variant_check, NoneType):
            self.options["DISABLE_VARIANT_CHECK"] = "TRUE" if disable_variant_check else "FALSE"
...
class CSVFormat(CopyFormat):
    ...
    def __init__(self, *,
        ...
        super().__init__()
        if record_delimiter:
            if len(str(record_delimiter).encode().decode('unicode_escape')) != 1 and record_delimiter != '\r\n':
                raise TypeError(
                    'Record Delimiter should be a single character.'
                )
            self.options['RECORD_DELIMITER'] = f"{repr(record_delimiter)}"
        if field_delimiter:
            if len(str(field_delimiter).encode().decode('unicode_escape')) != 1:
                raise TypeError(
                    'Field Delimiter should be a single character'
                )
        ...
        if error_on_column_mismatch:
            self.options["ERROR_ON_COLUMN_MISMATCH"] = str(error_on_column_mismatch).upper()
        if empty_field_as:
            if empty_field_as not in ['NULL', 'STRING', 'FIELD_DEFAULT']:
                raise TypeError('Empty Field As should be "NULL", "STRING" for "FIELD_DEFAULT".')
...
class NDJSONFormat(CopyFormat):
    ...
    def __init__(self, *,
        ...
        if missing_field_as:
            if missing_field_as not in ['ERROR', 'NULL', 'FIELD_DEFAULT', 'TYPE_DEFAULT']:
                raise TypeError('Missing Field As should be "ERROR", "NULL", "FIELD_DEFAULT" or "TYPE_DEFAULT".')
    ...
    def __init__(self, *,
        ...
        if missing_field_as:
            if missing_field_as not in ['ERROR', 'FIELD_DEFAULT']:
                raise TypeError('Missing Field As should be "ERROR" or "FIELD_DEFAULT".')
            self.options["MISSING_FIELD_AS"] = f"{missing_field_as}"
...
class AmazonS3(_StorageClause):
    ...
    def __init__(self, uri: str, access_key_id: str, secret_access_key: str, endpoint_url: str = None,
                 enable_virtual_host_style: bool = None, master_key: str = None,
                 region: str = None, security_token: str = None):
        r = urlparse(uri)
        if r.scheme != 's3':
